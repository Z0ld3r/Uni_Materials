...
public static class GrossHack
{
    ...
    public static (bool Found, Goldsmith Goldsmith) HelpLeastProfitableNice(this GoldsmithRegistry w, int ex) =>
        w.TranslateBoolXType<GoldsmithRegistry, Goldsmith>("HelpLeastProfitable",
            AllowedSignatures.BothOutParam | AllowedSignatures.OutParamNullable | AllowedSignatures.ReturnBoolOutParam |
            AllowedSignatures.ReturnNullable | AllowedSignatures.ReturnTuple,
            BindingFlags.Public | BindingFlags.Instance,
            [typeof(int)], [ex]);
    ...
    public static T GetSingletonInstance<T>() => (T)GetSingletonInstance(typeof(T)); // static extensions are a thing in only C# 14
